...
odei = [1 2 3];            % varibles for the ode solver
augi = [];                   % variables to be augmented
dyno = [1 2 3];            % variables to be predicted (and known to loss)
angi = [];                  % angle variables
dyni = [1 2 3];          % variables that serve as inputs to the dynamics GP
poli = [1 2 3];          % variables that serve as inputs to the policy
difi = [1 2 3];            % variables that are learned via differences


% 2. Set up the scenario
dt = 0.10;                         % [s] sampling time
T = 0.10; %4.0;                           % [s] initial prediction horizon time
...
policy.fcn = @(policy,m,s)conCat(@congp,@gSat,policy,m,s);% controller 
                                                          % representation
policy.maxU = 0.3;                                         % max. amplitude of 
                                                          % control
